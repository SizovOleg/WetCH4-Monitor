/**
 * @fileoverview Зональная статистика для статьи.
 *
 * Для каждой из 8 природных зон ЗСР:
 *   1. Доля болот (%), T_air, NDVI, XCH₄ wet/nonwet, ΔCH₄
 *   2. Сезонный ход ΔCH₄ по каждой зоне (48 записей: 8 зон × 6 мес)
 *
 * Средние за май–октябрь 2019–2025.
 * Все scale/tileScale адаптированы под сложную геометрию WSP shapefile.
 */

// ============================================================
// Imports
// ============================================================

var c = require('users/ntcomz18_sand/wetch4_ws:gee/lib/constants');
var palettes = require('users/ntcomz18_sand/wetch4_ws:gee/lib/palettes');
var tropomiModule = require('users/ntcomz18_sand/wetch4_ws:gee/02_tropomi_monthly');

// ============================================================
// Подготовка данных
// ============================================================

var FULL_AOI = c.FULL_AOI.simplify(1000);
var WSP = c.WSP;

// CGLS маски
var cgls = ee.Image(c.CGLS_COLLECTION).select('discrete_classification');
var wetlandMask = cgls.eq(90).clip(FULL_AOI);
var nonWetlandMask = wetlandMask.not().clip(FULL_AOI);

// TROPOMI monthly → общее среднее за все годы
var monthlyXCH4 = tropomiModule.buildMonthlyCollection(
  FULL_AOI, c.START_DATE, c.END_DATE
).map(function(img) { return ee.Image(img).clip(FULL_AOI); });

var meanXCH4 = monthlyXCH4.mean().rename('xch4');

// ERA5 T_air — среднее за тёплый период 2019–2025
var years = ee.List.sequence(2019, 2025);

var meanTemp = ee.ImageCollection(years.map(function(y) {
  y = ee.Number(y);
  return ee.ImageCollection(c.ERA5_COLLECTION)
    .filterDate(ee.Date.fromYMD(y, 5, 1), ee.Date.fromYMD(y, 11, 1))
    .select('temperature_2m')
    .mean().subtract(273.15).rename('t_air');
})).mean();

// MODIS NDVI — среднее за тёплый период 2019–2025
var meanNDVI = ee.ImageCollection(years.map(function(y) {
  y = ee.Number(y);
  return ee.ImageCollection('MODIS/061/MOD13A1')
    .filterDate(ee.Date.fromYMD(y, 5, 1), ee.Date.fromYMD(y, 11, 1))
    .select('NDVI')
    .mean().multiply(0.0001).rename('ndvi');
})).mean();

print('═══ ZONAL STATISTICS FOR PAPER ═══');
print('Monthly composites:', monthlyXCH4.size());

// ============================================================
// Часть 1: Сводная таблица (8 записей × 10 показателей)
// ============================================================

var zoneNames = ee.Dictionary({
  '1': 'Tundra', '2': 'Forest-tundra', '3': 'Northern taiga',
  '4': 'Middle taiga', '5': 'Southern taiga', '6': 'Subtaiga',
  '7': 'Forest-steppe', '8': 'Steppe'
});

var zoneStats = WSP.map(function(zone) {
  var geom = zone.geometry();
  var zoneId = ee.Number(zone.get('ID')).int();

  // Площадь зоны (scale:1000 для сложной геометрии)
  var totalArea = ee.Number(ee.Image.pixelArea().reduceRegion({
    reducer: ee.Reducer.sum(),
    geometry: geom, scale: 1000, maxPixels: 1e11, tileScale: 8
  }).values().get(0));

  // Площадь болот (scale:1000)
  var wetAreaRaw = wetlandMask.multiply(ee.Image.pixelArea()).reduceRegion({
    reducer: ee.Reducer.sum(),
    geometry: geom, scale: 1000, maxPixels: 1e11, tileScale: 8
  }).values().get(0);
  var wetArea = ee.Number(ee.Algorithms.If(wetAreaRaw, wetAreaRaw, 0));

  // Доля болот (%)
  var wetFraction = wetArea.divide(totalArea).multiply(100);

  // T_air над болотами (°C)
  var tAirWet = meanTemp.updateMask(wetlandMask).reduceRegion({
    reducer: ee.Reducer.mean(),
    geometry: geom, scale: 11000, maxPixels: 1e10, tileScale: 8
  }).get('t_air');

  // NDVI болот
  var ndviWet = meanNDVI.updateMask(wetlandMask).reduceRegion({
    reducer: ee.Reducer.mean(),
    geometry: geom, scale: 500, maxPixels: 1e10, tileScale: 8
  }).get('ndvi');

  // XCH₄ болот
  var xch4Wet = meanXCH4.updateMask(wetlandMask).reduceRegion({
    reducer: ee.Reducer.mean(),
    geometry: geom, scale: 7000, maxPixels: 1e10, tileScale: 8
  }).get('xch4');

  // XCH₄ не-болот
  var xch4NonWet = meanXCH4.updateMask(nonWetlandMask).reduceRegion({
    reducer: ee.Reducer.mean(),
    geometry: geom, scale: 7000, maxPixels: 1e10, tileScale: 8
  }).get('xch4');

  // ΔCH₄ (с null-защитой для степи)
  var deltaCH4 = ee.Algorithms.If(
    ee.Algorithms.IsEqual(xch4Wet, null), 0,
    ee.Algorithms.If(ee.Algorithms.IsEqual(xch4NonWet, null), 0,
      ee.Number(xch4Wet).subtract(ee.Number(xch4NonWet))
    )
  );

  return zone.set({
    'zone_name': zoneNames.get(zoneId.format('%d')),
    'zone_id': zoneId,
    'total_area_km2': totalArea.divide(1e6),
    'wetland_area_km2': wetArea.divide(1e6),
    'wetland_fraction_pct': wetFraction,
    't_air_wetland_c': ee.Algorithms.If(tAirWet, tAirWet, 0),
    'ndvi_wetland': ee.Algorithms.If(ndviWet, ndviWet, 0),
    'xch4_wetland_ppb': ee.Algorithms.If(xch4Wet, xch4Wet, 0),
    'xch4_nonwetland_ppb': ee.Algorithms.If(xch4NonWet, xch4NonWet, 0),
    'delta_ch4_ppb': deltaCH4
  });
});

var sorted = zoneStats.sort('zone_id');
print('Zone stats:', sorted);

// ============================================================
// Часть 2: Графики (4 шт)
// ============================================================

print(ui.Chart.feature.byFeature(sorted, 'zone_name', 'wetland_fraction_pct')
  .setChartType('ColumnChart')
  .setOptions({
    title: 'Wetland fraction by natural zone (%)',
    hAxis: {slantedText: true}, vAxis: {title: 'Wetland (%)'},
    colors: ['#1f77b4'], legend: 'none'
  }));

print(ui.Chart.feature.byFeature(sorted, 'zone_name', 'delta_ch4_ppb')
  .setChartType('ColumnChart')
  .setOptions({
    title: 'Enhancement \u0394CH\u2084 by natural zone (ppb)',
    hAxis: {slantedText: true}, vAxis: {title: '\u0394CH\u2084 (ppb)', baseline: 0},
    colors: ['#d62728'], legend: 'none'
  }));

print(ui.Chart.feature.byFeature(sorted, 't_air_wetland_c', 'delta_ch4_ppb')
  .setChartType('ScatterChart')
  .setOptions({
    title: '\u0394CH\u2084 vs T_air by natural zone',
    hAxis: {title: 'T_air (\u00B0C)'}, vAxis: {title: '\u0394CH\u2084 (ppb)'},
    pointSize: 8, trendlines: {0: {color: 'red', showR2: true}}
  }));

print(ui.Chart.feature.byFeature(sorted, 'wetland_fraction_pct', 'delta_ch4_ppb')
  .setChartType('ScatterChart')
  .setOptions({
    title: '\u0394CH\u2084 vs Wetland fraction by zone',
    hAxis: {title: 'Wetland (%)'}, vAxis: {title: '\u0394CH\u2084 (ppb)'},
    pointSize: 8, trendlines: {0: {color: 'red', showR2: true}}
  }));

// ============================================================
// Часть 3: Сезонный ход по зонам (48 записей: 8 × 6)
// ============================================================

var months = ee.List([5, 6, 7, 8, 9, 10]);

var zonalSeasonal = WSP.map(function(zone) {
  var geom = zone.geometry();
  var zoneId = ee.Number(zone.get('ID')).int();
  var zoneName = zoneNames.get(zoneId.format('%d'));

  var seasonalData = months.map(function(m) {
    var monthMean = monthlyXCH4.filter(ee.Filter.eq('month', m)).mean();

    var xWet = monthMean.updateMask(wetlandMask).reduceRegion({
      reducer: ee.Reducer.mean(),
      geometry: geom, scale: 7000, maxPixels: 1e10, tileScale: 8
    }).get('xch4');

    var xNon = monthMean.updateMask(nonWetlandMask).reduceRegion({
      reducer: ee.Reducer.mean(),
      geometry: geom, scale: 7000, maxPixels: 1e10, tileScale: 8
    }).get('xch4');

    var delta = ee.Algorithms.If(
      ee.Algorithms.IsEqual(xWet, null), 0,
      ee.Algorithms.If(ee.Algorithms.IsEqual(xNon, null), 0,
        ee.Number(xWet).subtract(ee.Number(xNon))
      )
    );

    return ee.Feature(ee.Geometry.Point([0, 0]), {
      zone_id: zoneId, zone_name: zoneName, month: m,
      xch4_wetland: ee.Algorithms.If(xWet, xWet, 0),
      xch4_nonwetland: ee.Algorithms.If(xNon, xNon, 0),
      delta_ch4: delta
    });
  });

  return ee.FeatureCollection(seasonalData);
}).flatten();

print('Zonal seasonal (48 records):', zonalSeasonal.size());

// ============================================================
// Часть 4: Per-zone charts
// Шаг 1: _SHOW_ZONE_CHARTS = false → запустить Export asset (внизу)
// Шаг 2: _SHOW_ZONE_CHARTS = true → графики из asset мгновенно
// ============================================================

var _SHOW_ZONE_CHARTS = false;

if (_SHOW_ZONE_CHARTS) {

var ROOT = 'projects/nodal-thunder-481307-u1/assets/';
var zsAsset = ee.FeatureCollection(ROOT + 'zonal_seasonal');

var zoneNameList = ['', 'Tundra', 'Forest-tundra', 'Northern taiga',
  'Middle taiga', 'Southern taiga', 'Subtaiga', 'Forest-steppe', 'Steppe'];

for (var zid = 1; zid <= 8; zid++) {
  var zoneData = zsAsset.filter(ee.Filter.eq('zone_id', zid));
  var name = zoneNameList[zid];

  print(ui.Chart.feature.byFeature(zoneData, 'month',
      ['xch4_wetland', 'xch4_nonwetland'])
    .setChartType('LineChart')
    .setOptions({
      title: name + ': XCH\u2084 Wetlands vs Non-wetlands',
      hAxis: {title: 'Month', ticks: [5,6,7,8,9,10]},
      vAxis: {title: 'XCH\u2084 (ppb)'},
      series: {
        0: {color: 'cyan', lineWidth: 2, pointSize: 5, labelInLegend: 'Wetlands'},
        1: {color: 'darkgreen', lineWidth: 2, pointSize: 5, labelInLegend: 'Non-wetlands'}
      }
    }));

  print(ui.Chart.feature.byFeature(zoneData, 'month', 'delta_ch4')
    .setChartType('ColumnChart')
    .setOptions({
      title: name + ': Seasonal \u0394CH\u2084',
      hAxis: {title: 'Month', ticks: [5,6,7,8,9,10]},
      vAxis: {title: '\u0394CH\u2084 (ppb)', baseline: 0},
      colors: ['#1f77b4'], legend: 'none'
    }));
}

} // end _SHOW_ZONE_CHARTS

// ============================================================
// Часть 5: Карта
// ============================================================

Map.centerObject(FULL_AOI, 5);
var zonesImage = WSP.reduceToImage({
  properties: ['ID'], reducer: ee.Reducer.first()
}).rename('zone_id').clip(FULL_AOI);
Map.addLayer(zonesImage, {min: 1, max: 8, palette: c.ZONE_PALETTE}, 'Natural zones');
Map.addLayer(WSP.style({color: 'black', fillColor: '00000000', width: 1}), {}, 'Zone boundaries');
Map.addLayer(c.MUKHRINO, {color: 'yellow'}, 'Mukhrino');
Map.addLayer(c.BAKCHAR, {color: 'red'}, 'Bakchar');
Map.addLayer(c.ZOTTO, {color: 'magenta'}, 'ZOTTO');

// ============================================================
// Часть 6: Exports
// ============================================================

Export.table.toDrive({
  collection: sorted,
  description: 'wetch4_zonal_statistics_8zones',
  fileFormat: 'CSV',
  selectors: ['zone_id', 'zone_name', 'total_area_km2', 'wetland_area_km2',
              'wetland_fraction_pct', 't_air_wetland_c', 'ndvi_wetland',
              'xch4_wetland_ppb', 'xch4_nonwetland_ppb', 'delta_ch4_ppb']
});

Export.table.toDrive({
  collection: zonalSeasonal,
  description: 'wetch4_zonal_seasonal',
  fileFormat: 'CSV',
  selectors: ['zone_id', 'zone_name', 'month',
              'xch4_wetland', 'xch4_nonwetland', 'delta_ch4']
});

// Asset для мгновенных графиков (Часть 4)
Export.table.toAsset({
  collection: zonalSeasonal,
  description: 'asset_zonal_seasonal',
  assetId: 'projects/nodal-thunder-481307-u1/assets/zonal_seasonal'
});
